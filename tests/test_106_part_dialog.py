...
def setup_part_dialog(qtbot, filesystem):
    ...
    main = QMainWindow()
    dialog = PartDialog(main, datafile, None, Dialog.EDIT_ELEMENT)
...
def test_106_01_constructor(qtbot, filesystem):
    ...
    a Part
    """
    datafile, main, dialog = setup_part_dialog(qtbot, filesystem)
...
def test_106_04_fill_item_table(qtbot, filesystem):
    ...
    itemset = ItemSet(datafile, "part_number", part_number, "assembly")
...
def test_106_05_fill_dialog_fields(qtbot, filesystem):
    ...
    dialog = PartDialog(main, datafile, None, Dialog.ADD_ELEMENT)
    ...
    dialog = PartDialog(main, datafile, part_value_set[0][0], Dialog.ADD_ELEMENT)
    dialog.fill_dialog_fields()
    ...
    assert (
        ...
        == Source(datafile, part_value_set[0][2]).get_source()
    )
    ...
    dialog = PartDialog(main, datafile, part_value_set[1][0], Dialog.ADD_ELEMENT)
    dialog.fill_dialog_fields()
    ...
    assert (
        dialog.form.source_combo.currentText()
        == Source(datafile, part_value_set[1][2]).get_source()
    )
...
def test_106_04_action_source_changed(qtbot, filesystem):
    ...
    dialog.set_combo_box_selections(
        ...
        SourceSet(datafile).build_option_list("source"),
        None,
    )
...
def test_106_09_clear_dialog(qtbot, filesystem):
    ...
    dialog = PartDialog(main, datafile, part_value_set[0][0], Dialog.EDIT_ELEMENT)
    assert dialog.form.record_id_edit.text() == str(part_value_set[0][0])
    dialog.clear_dialog()
    assert dialog.form.part_number_combo.count() == len(part_value_set)
    assert dialog.form.part_number_edit.text() == ""
...
def test_106_10_action_part_number_combo_changed(qtbot, filesystem):
    ...
    part1 = Part(datafile, part_value_set[1][1], "part_number")
    part2 = Part(datafile, part_value_set[0][1], "part_number")
    dialog = PartDialog(main, datafile, part1.get_record_id())
    ...
    assert (
        dialog.form.source_combo.currentText()
        == Source(datafile, part2.get_source()).get_source()
    )
    ...
    assert (
        ...
        == OrderLineSet(
            datafile, "part_number", part2.get_part_number()
        ).get_number_elements()
    )
...
def test_106_15_action_delete(qtbot, filesystem, mocker):
    ...
    # delete an part
    dialog = PartDialog(main, datafile, 1954, Dialog.EDIT_ELEMENT)
    assert dialog.form.record_id_edit.text() == "1954"
    partset_size = PartSet(datafile).get_number_elements()
    dialog.form.delete_button.click()
    assert dialog.form.record_id_edit.text() == "0"
    new_partset_size = PartSet(datafile).get_number_elements()
    ...
    dialog = PartDialog(main, datafile, 184, Dialog.EDIT_ELEMENT)
    partset_size = PartSet(datafile).get_number_elements()
    dialog.action_delete()
    new_partset_size = PartSet(datafile).get_number_elements()
    assert partset_size == new_partset_size
