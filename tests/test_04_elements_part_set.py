...
def load_parts_table(dbref):
    ...
    value_set = [['1786', 'X081', 'Local Purchase', 'Rivet, Pop,1/8, Dia, 1/4 grip, Aluminum', '' ],
                 ...
                 ['1795', '282-385', 'Moss USA', 'Grommet, 9/16 OD x 3/16 ID', 'License Plate Wires' ],
...
def test_04_08_selected_rows(create_parts_table):
    ...
    count_result = dbref.sql_query("SELECT COUNT(*) FROM " + part_set.get_table() + " WHERE source = 'Victoria British'")
    count = dbref.sql_fetchrow(count_result)['COUNT(*)']
    assert count == len(part_set.get_property_set())
    assert count == 3
    close_database(dbref)
